...
function renderRecursoEditor(moduloId, diaNum, ri, r) {
  ...
  if (esEval) {
    ...
    const bancoColapsadoId = `banco-body-${moduloId}-${diaNum}-${ri}`;
    const bancoAbierto = r._bancoAbierto !== false; // abierto por defecto, se cierra solo si se colapsa manualmente
    extraHtml = `
      <div style="background:rgba(0,255,136,0.04);border:1px solid rgba(0,255,136,0.15);border-radius:10px;padding:14px;margin-top:10px;">
        <!-- Cabecera siempre visible con toggle -->
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;cursor:pointer;" onclick="toggleBanco('${bancoColapsadoId}',this)">
          <div>
            <div style="font-size:11px;font-weight:700;color:#00ff88;letter-spacing:0.06em;">BANCO DE PREGUNTAS · OPCIÓN MÚLTIPLE</div>
            <div style="font-size:11px;color:rgba(255,255,255,0.35);margin-top:2px;">${banco.length} preguntas · ${r.numPreguntas||5} por sesión · aleatorias</div>
          </div>
          <div style="display:flex;align-items:center;gap:10px;">
            <div style="display:flex;align-items:center;gap:6px;" onclick="event.stopPropagation()">
              <label style="font-size:11px;color:rgba(255,255,255,0.4);">Mostrar</label>
              <input type="number" min="1" max="${Math.max(banco.length,1)}" value="${r.numPreguntas||5}"
              ...
              <label style="font-size:11px;color:rgba(255,255,255,0.4);">de ${banco.length}</label>
            </div>
            <i class="fas fa-chevron-down" style="color:#00ff88;font-size:11px;transition:transform 0.2s;transform:rotate(${bancoAbierto?'180':'0'}deg);"></i>
          </div>
        </div>
        <!-- Cuerpo colapsable -->
        <div id="${bancoColapsadoId}" style="display:${bancoAbierto?'block':'none'};margin-top:12px;">
...
const BANCO_POR_DIA_CC = {
  1: [
    ...
    { enunciado:'Una empresa mide puntualmente su OTIF pero nunca ha medido su CES. ¿Qué punto ciego tiene en su gestión?', opciones:['No tiene punto ciego: el OTIF cubre todo lo que el cliente valora en la entrega','Desconoce cuánto esfuerzo le cuesta al cliente interactuar con su cadena; puede estar reteniendo clientes insatisfechos sin saberlo','Solo necesita medir el NPS para complementar el OTIF; el CES es redundante','El punto ciego está en los costos logísticos, no en la experiencia del cliente'], correcta:1 },
    { enunciado:'¿Cuál es la señal más temprana de que una cadena eficiente está perdiendo relevancia para el cliente conectado?', opciones:['Caída en el volumen de ventas trimestral','Aumento en quejas por falta de visibilidad o comunicación proactiva, aunque el OTIF se mantenga alto','Incremento en el costo de transporte por unidad','Reducción en el número de SKUs activos del portafolio'], correcta:1 }
  ],
  2: [
    ...
    { enunciado:'El costo de atención de Refresco Ágil ($5.80/hl) es 53% menor al de Odyssey ($12.40/hl). ¿Qué implicación estratégica tiene esto para Odyssey más allá del costo?', opciones:['Refresco Ágil sacrifica calidad por costo; Odyssey tiene ventaja en servicio premium','Refresco Ágil puede ofrecer mejor precio o mayor margen manteniendo el mismo precio; Odyssey perderá clientes sensibles al precio si no transforma su operación','La diferencia de costo es solo contable y no afecta la competitividad','Solo impacta si los clientes conocen la estructura de costos del proveedor'], correcta:1 },
    { enunciado:'¿Por qué la "consumerización" del B2B es el cambio más disruptivo para distribuidores tradicionales?', opciones:['Porque obliga a tener presencia en redes sociales y plataformas digitales','Porque los compradores B2B ahora trasladan sus expectativas como consumidores (Amazon, apps) a sus interacciones comerciales, exigiendo velocidad, visibilidad y autoservicio','Porque reduce el rol del ejecutivo de ventas y elimina la relación personal con el cliente','Porque requiere invertir en tecnología que los distribuidores no pueden costear'], correcta:1 }
  ],
  3: [
    ...
    { enunciado:'El CES promedio del journey de Cadena Norte es 3.27/7. Si se mejora a 2.0/7, ¿qué impacto es más esperable en el negocio?', opciones:['Incremento en el volumen de pedidos por mayor facilidad en el proceso','Mayor lealtad, menor probabilidad de abandono y reducción en el costo de servicio al cliente por menos contactos de soporte','Aumento en el ticket promedio por mayor satisfacción con la experiencia','Solo mejora la percepción, sin impacto medible en comportamiento de compra'], correcta:1 },
    { enunciado:'¿Por qué priorizar TP3 (Notificación) sobre TP5 (Discrepancias) aunque el CES del TP5 es más bajo?', opciones:['No hay razón; siempre se debe priorizar el touchpoint con el CES más bajo','El TP3 afecta a todos los pedidos (100%) mientras el TP5 solo impacta los pedidos con error (23%); el volumen justifica priorizar TP3 aunque el CES sea ligeramente mejor','El TP3 es más barato de resolver tecnológicamente que el TP5','El TP5 es responsabilidad del proveedor de transporte, no de Odyssey'], correcta:1 }
  ],
  4: [
    ...
    { enunciado:'¿Qué diferencia a una estrategia de resiliencia proactiva de una reactiva en cadena de suministro?', opciones:['La proactiva es más costosa y solo aplica a grandes empresas con presupuesto para redundancia','La proactiva identifica y mitiga vulnerabilidades antes de la disrupción; la reactiva responde después de sufrirla, cuando el costo ya se materializó','La reactiva es suficiente si se tiene un buen equipo de gestión de crisis','Solo difieren en el momento de implementación; los costos y resultados son equivalentes'], correcta:1 },
    { enunciado:'Odyssey tiene 3 de 4 insumos críticos con proveedor único. ¿Cuál debe proteger primero si solo puede actuar sobre uno este año?', opciones:['El de mayor costo unitario, porque su impacto financiero directo es mayor','El concentrado de saborizante (28% costo, lead time 90 días) porque combina alto costo, lead time largo e impacto en el producto final que afecta toda la línea de producción','El que tiene el proveedor geográficamente más lejos para reducir riesgo de interrupción logística','El que tiene mayor rotación de inventario porque se agotará primero en una disrupción'], correcta:1 }
  ],
  5: [
    ...
    { enunciado:'¿Por qué la sostenibilidad del modelo omnicanal depende más de los procesos que de la tecnología?', opciones:['Porque la tecnología es demasiado costosa para mantener actualizada en el largo plazo','Porque sin procesos de asignación de inventario, gestión de devoluciones y coordinación entre canales definidos, la tecnología no puede mantener la promesa de disponibilidad consistente','Porque los clientes valoran la relación humana por encima de la experiencia digital','Porque la tecnología omnicanal está disponible para todos los competidores por igual'], correcta:1 },
    { enunciado:'Si la tasa de quiebre de e-commerce baja de 11.4% a 3.1% tras la integración de inventario, ¿cuál es el impacto más relevante para la retención de clientes?', opciones:['Se reduce el costo de atención de quejas por stockout','El cliente digital que no encuentra el producto en su primer intento tiene alta probabilidad de no regresar; reducir el quiebre protege directamente la base de clientes del canal','Solo mejora la percepción de la marca sin impacto medible en ventas repetidas','El impacto principal es en la eficiencia operativa del almacén, no en la retención'], correcta:1 }
  ],
  6: [
    ...
    { enunciado:'El costo promedio de gestión de un SKU Tier C es $67,200/año vs. $18,400 de un SKU Tier A. ¿Qué explica esta diferencia de 3.6x?', opciones:['Los SKUs Tier C son más nuevos y requieren mayor inversión en marketing y distribución','Las corridas cortas de producción, alta frecuencia de setup, baja rotación y mayor esfuerzo de gestión de pedidos especiales multiplican el costo unitario de operación','Los SKUs Tier A tienen economías de escala en transporte que reducen su costo proporcionalmente','La diferencia es contable; en la operación real el costo no varía significativamente entre tiers'], correcta:1 },
    { enunciado:'¿Por qué el fill rate del 71% de SKUs Tier C (vs. 94% del resto) es un problema para el cliente aunque esos SKUs representen solo 2% de ventas?', opciones:['No es un problema: el cliente acepta menor disponibilidad en productos de nicho','Cuando el cliente pide un SKU Tier C y no está disponible, experimenta una promesa rota que daña la confianza en la marca independientemente del valor económico del pedido','Solo importa si el cliente hace devoluciones o cancelaciones por el stockout','El fill rate del 71% es el estándar de la industria para SKUs de baja rotación'], correcta:1 }
  ],
  7: [
    ...
    { enunciado:'Un distribuidor dice: "Prefiero llamar a mi ejecutivo de ventas; el proceso digital es más lento." ¿Qué revela esto sobre la implementación del portal?', opciones:['Que los distribuidores mayores de cierta edad nunca adoptarán los canales digitales','Que el portal no está generando valor suficiente para superar la fricción del cambio; el rediseño debe simplificar más o la adopción fallará aunque la tecnología esté disponible','Que el ejecutivo de ventas debe ser eliminado del proceso para forzar la adopción digital','Que el cliente tiene razón y el canal telefónico es siempre superior al digital para B2B'], correcta:1 },
    { enunciado:'¿Cuál es la métrica correcta para evaluar el éxito del portal de autoservicio de Odyssey 6 meses después de su implementación?', opciones:['El número de distribuidores registrados en el portal','La tasa de adopción activa (% de pedidos colocados por portal vs. canal tradicional) combinada con el NPS de distribuidores y la reducción en errores de pedido','El tiempo que tardó en implementarse el portal y el costo del proyecto','El nivel de satisfacción del equipo de TI con la plataforma tecnológica seleccionada'], correcta:1 }
  ],
  8: [
    ...
    { enunciado:'Odyssey subió el NPS de 51 a 58 en 9 meses pero el costo de servicio reactivo sigue en $1.26M/año. ¿Qué conclusión se extrae?', opciones:['El aumento del NPS demuestra que la estrategia está funcionando; el costo reducirá naturalmente después','Las intervenciones mejoraron la satisfacción pero no atacaron las causas de las quejas; el equipo sigue gestionando los mismos problemas, solo con mejor actitud','El NPS no es un indicador confiable para medir el éxito de la transformación de la cadena','El costo de $1.26M es aceptable si el NPS supera 70'], correcta:1 },
    { enunciado:'¿Cómo diferencia un tablero de CX efectivo entre un indicador de resultado y un indicador de proceso en el contexto de Odyssey?', opciones:['Los indicadores de resultado son siempre financieros; los de proceso son siempre operativos','NPS es un indicador de resultado (refleja la experiencia acumulada); FCR por tipo de queja es un indicador de proceso (señala qué está fallando en la operación antes de que impacte al NPS)','No es posible diferenciarlos en la práctica; todos los KPIs miden resultado','Los indicadores de proceso solo los usa el equipo de operaciones; el tablero de CX solo debe mostrar indicadores de resultado'], correcta:1 }
  ],
  9: [
    ...
    { enunciado:'Si Odyssey implementa IoT en los 240 equipos de frío pero no integra los datos con su sistema de servicio al cliente, ¿qué valor se pierde?', opciones:['Ninguno: el valor del IoT está en detectar la falla, no en la integración con otros sistemas','Se pierde la capacidad de cerrar el loop: los datos del equipo no alimentan el historial del cliente, impidiendo el servicio proactivo y el aprendizaje continuo para mejorar el modelo de servicio','Solo afecta la velocidad de respuesta del técnico; el cliente no percibe diferencia','Se pierde el argumento de venta del sistema IoT ante el equipo directivo'], correcta:1 },
    { enunciado:'¿Por qué la métrica "primera visita resuelta" (FVR) es más relevante que "tiempo de respuesta" para medir el servicio de campo conectado?', opciones:['Porque el tiempo de respuesta es más difícil de medir con precisión','Porque llegar rápido sin resolver el problema incrementa el CES y el costo; el cliente valora que el problema quede resuelto en la primera visita sobre la velocidad de llegada','Porque el tiempo de respuesta ya está regulado en el contrato de servicio','Porque la FVR es el único KPI que los clientes comprenden y pueden validar fácilmente'], correcta:1 }
  ],
  10: [
    ...
    { enunciado:'¿Cuál es el error más común al intentar replicar la transformación de Odyssey en otra empresa?', opciones:['Copiar el orden exacto de las intervenciones sin adaptarlas al contexto operativo propio','Enfocarse solo en la tecnología (portal, IoT, notificaciones) sin mapear primero el Customer Journey y las causas raíz de fricción específicas de esa operación','Medir el NPS antes de iniciar cualquier intervención','Incluir al área de operaciones en el diseño de las soluciones de experiencia del cliente'], correcta:1 },
    { enunciado:'¿Qué aprendizaje del módulo es más aplicable a una empresa que aún opera con modelo 100% tradicional (sin e-commerce, sin portal)?', opciones:['El módulo no aplica; está diseñado para empresas con presencia digital','Mapear el Customer Journey actual e identificar los 2-3 touchpoints con mayor fricción: ese diagnóstico es válido en cualquier modelo y genera el roadmap de transformación','Implementar primero el canal e-commerce antes de mejorar cualquier otro aspecto de la cadena','Esperar a que el mercado exija la transformación antes de invertir en cambios sin demanda comprobada'], correcta:1 }
  ]
};

// ── Toggle del banco de preguntas ──
function toggleBanco(bodyId, header) {
  const body = document.getElementById(bodyId);
  if (!body) return;
  ...
  // Persistir estado en el recurso para que survive re-renders
  const parts = bodyId.replace('banco-body-','').split('-');
  // bodyId = banco-body-{moduloId}-{diaNum}-{ri}  → last token is ri, second-last is diaNum, rest is moduloId
  const ri = parseInt(parts[parts.length-1]);
  const diaNum = parseInt(parts[parts.length-2]);
  const moduloId = parts.slice(0, parts.length-2).join('-');
  const m = mockModulosGrid.find(x=>x.id===moduloId);
  const d = m?.contenido.find(d=>d.dia===diaNum);
  if (d?.recursos[ri]) d.recursos[ri]._bancoAbierto = !isOpen;
}
